...
const INITIAL_FORM = { category: null, amount: "", description: "", expense_date: "" };
...
function ExpensesPage() {
  ...
  const loadData = async (currentFilters = appliedFilters) => {
    ...
    try {
      const [expenseData, categoryData, summaryData] = await Promise.all([
        fetchExpenses(buildFilterParams(currentFilters)),
        fetchExpenseCategories(),
        fetchExpenseSummary(),
      ]);
      setItems(Array.isArray(expenseData) ? expenseData : expenseData?.results || []);
      setCategories(Array.isArray(categoryData) ? categoryData : categoryData?.results || []);
      setSummary(summaryData || { weekly_expenses: 0, monthly_expenses: 0, yearly_expenses: 0 });
  ...
  const submitExpense = async (event) => {
    ...
    try {
      ...
      if (editing) {
        ...
        notify("success", t("confirmation"), t("expenseUpdatedSuccess"));
      } else {
        await createExpense(payload);
        notify("success", t("confirmation"), t("expenseCreatedSuccess"));
      }
      setOpen(false);
      setEditing(null);
  ...
  const onDelete = async (item) => {
    try {
      ...
      notify("success", t("confirmation"), t("expenseDeletedSuccess"));
      await loadData(appliedFilters);
    } catch (error) {
      notify("error", t("information"), error.message);
    }
  ...
  const submitCategory = async (event) => {
    ...
    try {
      ...
      notify("success", t("confirmation"), t("expenseCategoryCreatedSuccess"));
      await loadData(appliedFilters);
    } catch (error) {
      notify("error", t("information"), error.message);
    } finally {
  ...
  const handleExportPdf = async () => {
    setExporting(true);
    try {
      const { blob, fileName } = await exportExpensesPdf(buildFilterParams(appliedFilters));
  ...
  const table = useMemo(() => {
    const columns = [
      ...
      { Header: t("time"), accessor: "expense_date", align: "left" },
      { Header: t("recordedBy"), accessor: "recorded_by_username", align: "left" },
    ...
    const rows = items.map((item) => ({
      ...
      expense_date: <MDTypography variant="caption">{item.expense_date || "-"}</MDTypography>,
      recorded_by_username: (
        <MDTypography variant="caption">{item.recorded_by_username || "-"}</MDTypography>
      ),
      actions: canWrite ? (
    ...
    <DashboardLayout>
            ...
            <Card>
              <MDBox p={3}>
                <MDBox display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                  <MDBox>
                    <MDTypography variant="h6">{t("expenses")}</MDTypography>
                ...
                <Grid container spacing={2} mb={2}>
                  <Grid item xs={12} md={4}>
                    <MDBox p={2} borderRadius="lg" bgColor="light">
                      <MDTypography variant="button">{t("weeklyExpenses")}</MDTypography>
                      <MDTypography variant="h5">
                        {Number(summary.weekly_expenses || 0).toLocaleString()} {t("rwf")}
                      </MDTypography>
                    </MDBox>
                  </Grid>
                  <Grid item xs={12} md={4}>
                    <MDBox p={2} borderRadius="lg" bgColor="light">
                      <MDTypography variant="button">{t("monthlyExpenses")}</MDTypography>
                      <MDTypography variant="h5">
                        {Number(summary.monthly_expenses || 0).toLocaleString()} {t("rwf")}
                      </MDTypography>
                    </MDBox>
                  </Grid>
                  <Grid item xs={12} md={4}>
                    <MDBox p={2} borderRadius="lg" bgColor="light">
                      <MDTypography variant="button">{t("yearlyExpenses")}</MDTypography>
                      <MDTypography variant="h5">
                        {Number(summary.yearly_expenses || 0).toLocaleString()} {t("rwf")}
                      </MDTypography>
                    </MDBox>
                  </Grid>
                </Grid>
      ...
      {canWrite && (
        ...
        >
          ...
          <DialogContent sx={FORM_DIALOG_CONTENT_SX}>
            <MDBox component="form" id="expense-form" onSubmit={submitExpense} mt={1}>
      ...
      {canWrite && (
        ...
        >
          ...
          <DialogContent sx={FORM_DIALOG_CONTENT_SX}>
            <MDBox component="form" id="expense-category-form" onSubmit={submitCategory} mt={1}>
              <TextField
                fullWidth
                sx={FORM_FIELD_SX}
                label={t("type")}
